...
* **How it works**:
    ...
    * **Temporal Error Distributions (Temporal Outputs)**: For models outputting a sequence of labels (e.g., Seq2Seq), it provides a multi-dimensional assessment of prediction quality across the sequence length. This includes a Flattened Classification Report to compute aggregate metrics across all time steps, and Metrics Over Time Steps to generate a consolidated line plot visualizing F1-score, Precision, Recall, and ROC-AUC at each discrete timestamp. This diagnostic allows for identifying performance drift or specific time intervals where the model exhibits temporal bias or instability.